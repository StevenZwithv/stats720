...
Below is the model matrix for Period*Treatment. The model matrix of Period*Treatment meaning we are including the two principle terms and also the interaction term which is the product of two dichotomous variables (0s and 1s). We then should be having a model matrix with columns of intercept, PeriodBefore, TreatmentControl, and the PeriodBefore:TreatmentImpact interaction term.
